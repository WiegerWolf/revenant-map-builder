import { ChunkDecompressor } from './ChunkDecompressor.js';
import { ChunkHeader } from '../models/ChunkHeader.js';
import { BitmapFlags } from '../models/flags/BitmapFlags.js';
import { DrawModeFlags } from '../models/flags/DrawModeFlags.js';
import { BufferUtils } from '../utils/BufferUtils.js';

export class BitmapData {
    static readBitmap(stream, arrayBuffer) {
        const bitmap = this.readBitmapHeader(stream);
        ...
        this.processPixelData(bitmap, bitmapBuffer, bitmapStream);
        this.processPalette(bitmap, arrayBuffer);
        this.processAdditionalBuffers(bitmap, arrayBuffer, baseOffset);

        return bitmap;
    }

    static readBitmapHeader(stream) {
        return {
            ...
            ...this.readSizesAndOffsets(stream)
        };
    }

    static readSizesAndOffsets(stream) {
        const header = {};
        const offsetFields = ['alias', 'alpha', 'zbuffer', 'normal', 'palette'];
        
        for (const field of offsetFields) {
            const sizeField = `${field}size`;
            header[sizeField] = stream.readUint32();
            
            const offsetPos = stream.getPos();
            header[field] = stream.readUint32();
            
            if (header[field] !== 0) {
                header[field] += offsetPos;
            }
        }
    ...
    static processPixelData(bitmap, bitmapBuffer, bitmapStream) {
        if (bitmap.flags.bm_compressed) {
            this.processCompressedData(bitmap, bitmapBuffer, bitmapStream);
        } else {
            this.processUncompressedData(bitmap, bitmapBuffer);
        }
    }

    static processCompressedData(bitmap, bitmapBuffer, bitmapStream) {
        ...
        bitmap.data = this.allocatePixelBuffer(bitmap);

        this.processChunks(bitmap, mainHeader, bitmapBuffer, bitmapStream);
    }

    static allocatePixelBuffer(bitmap) {
        if (bitmap.flags.bm_8bit) {
            ...
            return new Uint16Array(bitmap.width * bitmap.height);
        } else if (bitmap.flags.bm_24bit) {
            return new Uint8Array(bitmap.width * bitmap.height * 3);
        } else if (bitmap.flags.bm_32bit) {
            return new Uint32Array(bitmap.width * bitmap.height);
        }
    }

    static processChunks(bitmap, mainHeader, bitmapBuffer, bitmapStream) {
        for (let y = 0; y < mainHeader.height; y++) {
            for (let x = 0; x < mainHeader.width; x++) {
                ...
                const blockWidth = this.calculateBlockWidth(x, bitmap, mainHeader);
                const blockHeight = this.calculateBlockHeight(y, bitmap, mainHeader);
                const destX = x * this.calculateBlockWidth(0, bitmap, mainHeader);
                const destY = y * this.calculateBlockHeight(0, bitmap, mainHeader);

                this.processChunkBlock(bitmap, mainHeader, bitmapBuffer, x, y, blockWidth, blockHeight, destX, destY);
            }
        }
    }

    static processChunkBlock(bitmap, mainHeader, bitmapBuffer, x, y, blockWidth, blockHeight, destX, destY) {
        const blockOffset = mainHeader.getBlockOffset(x, y);
        const blockSize = mainHeader.getBlockSize(x, y) || (bitmapBuffer.byteLength - blockOffset);
        ...
        const { data: decompressed } = ChunkDecompressor.decompressChunk(blockStream, blockWidth, blockHeight);
        this.copyDecompressedData(bitmap, decompressed, blockHeight, blockWidth, destX, destY);
    }

    static copyDecompressedData(bitmap, decompressed, blockHeight, blockWidth, destX, destY) {
        if (!bitmap.flags.bm_8bit) {
            console.warn('Unsupported bitmap format, will be implemented later');
            return;
        }

        for (let row = 0; row < blockHeight; row++) {
            const srcOffset = row * blockWidth;
            const destOffset = ((destY + row) * bitmap.width) + destX;
            bitmap.data.set(
                decompressed.subarray(srcOffset, srcOffset + blockWidth),
                destOffset
            );
        }
    }

    static processUncompressedData(bitmap, baseOffset) {
        if (bitmap.flags.bm_8bit) {
            bitmap.data = new Uint8Array(baseOffset, bitmap.datasize);
        } else if (bitmap.flags.bm_15bit || bitmap.flags.bm_16bit) {
            bitmap.data = new Uint16Array(baseOffset, bitmap.datasize / 2);
        } else if (bitmap.flags.bm_24bit) {
            bitmap.data = new Uint8Array(baseOffset, bitmap.datasize);
        } else if (bitmap.flags.bm_32bit) {
            bitmap.data = new Uint32Array(baseOffset, bitmap.datasize / 4);
        }
    }

    static processPalette(bitmap, arrayBuffer) {
        if (bitmap.palettesize <= 0 || bitmap.palette <= 0) {
            return;
        }

        const expectedSize = (256 * 2) + (256 * 4);
        ...
        if (bitmap.palette + expectedSize > arrayBuffer.byteLength) {
        ...
        for (let i = 0; i < 256; i++) {
            tempColors[i] = view.getUint16(bitmap.palette + i * 2, true);
            tempRGBColors[i] = view.getUint32(bitmap.palette + 512 + i * 4, true);
        }
    ...
    static processAdditionalBuffers(bitmap, arrayBuffer, baseOffset) {
        if (bitmap.flags.bm_compressed) {
            return;
        }

        // Additional buffers processing can be implemented here as needed
    ...
    static calculateBlockWidth(x, bitmap, mainHeader) {
        ...
        return x < mainHeader.width - 1 
            ? baseBlockWidth 
            : bitmap.width - baseBlockWidth * (mainHeader.width - 1);
    }

    static calculateBlockHeight(y, bitmap, mainHeader) {
        const baseBlockHeight = Math.floor(bitmap.height / mainHeader.height);
        return y < mainHeader.height - 1 
            ? baseBlockHeight 
            : bitmap.height - baseBlockHeight * (mainHeader.height - 1);
    }
}
